...
export default class AiExtractionAPI {
	...
	async scanGoogleCloud(accessToken: string): Promise<void | FetchError> {
		await this.fetch(
			this.getUrl(`GoogleCloud?` + new URLSearchParams({ accessToken: accessToken }))
		).catch((err) => {
	...
	async getAllOpenAIModels(apiKey: string): Promise<OpenAiModel[] | FetchError> {
	...
	async postSelectedOpenAIModels(models: OpenAiModel[]): Promise<void | FetchError> {
